            ...
            {Time_spinner ? <div className='flex justify-center h-screen items-center'>
                ...
                <div className='pl-[60px] w-[95%] flex flex-wrap '>
                    {PartMovie.map((part, index) => (
                        <button key={index} className={`bg-yellow-400 lg:w-[90px] w-[60px] lg:h-[40px] h-[40px] rounded-md mt-5 lg:ml-4 ml-1  font-bold hover:bg-yellow-500 mb-5 cursor-pointer
                         ${part.link_embed === seriesMovie ? 'bg-black ' : 'bg-yellow-400'} `} data-slug={part.slug} onClick={(e) => handlecheckData(e)}>{part.name}</button>
                    ))}
                </div>
            </div>}
        </div>
